...
const formatterSex: VxeColumnPropTypes.Formatter = ({ cellValue }) => {
  const item = usertable.sexList.find(item => item.value === cellValue)
...
<!--    <Alert />-->
    ...
    <PageMain>
      ...
      >
        ...
        <VxeColumn field="role" title="角色" />
        <VxeColumn field="sex" title="Sex" :filters="usertable.sexList" :formatter="formatterSex" />
        <VxeColumn field="age" title="Age" sortable :filters="usertable.ageList" :filter-multiple="false" :filter-method="filterAgeMethod" />
